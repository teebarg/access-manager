...
import * as accessClient from "../utils/access-client";
...
function ListItemProvider({ children }) {
  const { data } = useAuth();
  const [state, dispatch] = React.useReducer(listReducer, data.accesses);
...
function removeListItem(dispatch, id) {
  return accessClient.remove(id).then((data) => {
    dispatch({ type: "remove", id });
    return data;
  });
}

function addListItem(dispatch, accessData) {
  return accessClient.create(accessData).then(({ data }) => {
    dispatch({ type: "add", listItem: data });
    return data;
  }).catch(e => alert(e)) ;
}

function updateListItem(dispatch, listItemId, updates) {
  return accessClient.update(listItemId, updates).then((data) => {
